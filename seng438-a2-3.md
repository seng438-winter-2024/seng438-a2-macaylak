...
### Test Methods for `expand` method in Range class:

- **Test Method 1: testExpand**
  - **Description**: Test the expansion of a valid range with valid lower and upper margins.
  - **Partition**: Valid range, valid lower and upper margins.

- **Test Method 2: testExpandNullRange**
  - **Description**: Test behavior when null range object is passed in.
  - **Partition**: Null range.

- **Test Method 3: testExpandZeroMargins**
  - **Description**: Test behavior when both lower and upper margins are zero.
  - **Partition**: Both margins equal zero.

- **Test Method 4: testExpandZeroRange**
  - **Description**: Test behavior when the original range has zero length.
  - **Partition**: Zero length range.

### Test Methods for `equals` method in Range class:

- **Test Method 1: testNull**
  - **Description**: Test behavior when comparing with null.
  - **Partition**: Null comparison.

- **Test Method 2: testDifferentObject**
  - **Description**: Test behavior when comparing with a different object type.
  - **Partition**: Different object type comparison.

- **Test Method 3: testDifferentRange**
  - **Description**: Test behavior when comparing with a different range.
  - **Partition**: Different range comparison.

- **Test Method 4: testSameRange**
  - **Description**: Test behavior when comparing with the same range.
  - **Partition**: Same range comparison.
